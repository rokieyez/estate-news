...
| `shorts-draft.mp4` | 자막 카드를 타임코드대로 이어 붙인 쇼츠 초안(무음). 편집 출발점. 커밋되지 않고 Actions 아티팩트로만 |
...
- 데일리 워크플로에 07:40 KST 크론이 하나 더 있습니다. 첫 단계에서 `output/<오늘>/data.json`
...
3. **본문 복사** → 본문 칸에 붙여넣기 — 소제목·표·굵게·목록이 그대로 살아납니다.
   ...
   같은 핵심 수치가 썸네일 배지(`images.thumbnail(badge=)`)와 쇼츠 초안 첫 컷(`video.intro_svg`)에도 들어갑니다.
...
| 워크플로 | 언제 도는가 |
| --- | --- |
| 부동산 데일리 브리핑 | 매일 22:00 UTC(= 오전 7시 KST) + 수동 |
