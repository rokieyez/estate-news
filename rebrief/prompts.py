...
- visual 에는 그 구간에 무엇을 띄울지 적습니다. 촬영/편집자가 그대로 보고 작업할 수 있게 구체적으로.
  ...
  숫자·순서·비교를 그래픽으로 보여줄 줄은 끝에 ` + ` 를 붙이고 아래 모양 하나를 이어 적습니다.
  ...
  퍼센트가 아닌 값(표 수·건수)은 막대로 적고, 모양이 필요 없는 줄은 억지로 붙이지 마세요.
  예: "서울 아파트 단지 항공 스톡 + 큰 숫자: 서울 아파트값 / -0.03% / 3주째 하락 / 출처: 한국부동산원".
