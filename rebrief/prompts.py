...
def build_video_user(cfg: Config, stats: dict | None = None) -> str:
    ...
    return f"""위 브리핑{"과 아래 실거래 자료" if stats else ""}를 바탕으로 오늘 촬영할 {'영상 두 편(쇼츠·롱폼)' if longform_daily(cfg) else '쇼츠 한 편'}의 제작 자료를 만드세요.
{stats_context(stats)}

■ 쇼츠 ({shorts_sec}초)
- 브리핑에서 **가장 임팩트 있는 이슈 하나만** 고릅니다. 여러 개 담지 마세요.
- 발화 분량 합계 약 {shorts_chars}자 (분당 {cpm}자 기준). 이 분량을 넘기지 마세요.
- lines 는 **진행자가 읽을 대본**입니다. 이어 붙이면 그대로 내레이션이 되어야 합니다.
  화면에 한 번에 뜨는 만큼 끊되(한 줄 18자 이내), 한 문장을 두세 줄로 나눠도 됩니다.
- **제목투(개조식)로 쓰지 마세요.** 명사로 끝나는 조각을 늘어놓으면 대본이 아니라 헤드라인 목록이 됩니다.
  나쁜 예: "노원구 상계동 한양" / "8억 4천만원 거래" (제목을 늘어놓은 것)
  좋은 예: "노원구 상계동 한양이" → "직전보다 8천만원 오른" → "8억 4천만원에 팔렸습니다"
- 문장이 끝나는 줄은 **종결어미**(…습니다 / …죠 / …까요)로 맺고, 중간 줄은 조사·연결어미로 끝내
  다음 줄과 이어지게 합니다. 자막이므로 마침표는 찍지 않아도 됩니다.
- 명사로만 끝나는 줄이 전체의 3분의 2를 넘으면 다시 쓰세요.
- at 은 00:00 부터 시작해 실제 읽는 속도에 맞춰 매깁니다.
- **화면은 줄 단위가 아니라 씬 단위로 짭니다.** 영상은 모션그래픽 도구(motion-studio)로만 만듭니다 —
  촬영·스톡 영상·진행자 컷·블로그 첨부 그림(img-*.png)은 쓰지 않으니 지시하지 마세요 (사진은 아래 「사진 한 칸」 규칙으로만).
  뜻이 이어지는 자막 3~6줄을 한 씬으로 묶어 **한 씬 5~9초, 전체 7~9개 씬**으로 나눕니다 (맨 앞 훅 씬만 3초 안팎이어도 됩니다).
  visual 은 **각 씬의 첫 줄에만** 적고 같은 씬의 나머지 줄은 빈 문자열("")로 둡니다. 줄마다 화면을 바꾸면 1~2초짜리 컷이 되어 쓸 수 없습니다.
- visual 에는 그 씬 동안 화면에 **그래픽으로** 무엇을 그릴지 적습니다 — 무엇이 크게 보이고, 어느 말에서 무엇이 등장·강조되는지.
  나쁜 예: "서울 아파트 단지 항공 스톡", "진행자 클로즈업", "img-1-stat-card.png". 좋은 예: "서울 자치구 모식도에 송파·영등포·마포가 차례로 켜지고 구마다 줄어든 건수가 붙음".
  숫자·순서·비교가 있는 씬은 아래 모양 하나로 **시작**하고, 덧붙일 연출은 ` + ` 뒤에 적습니다.
  영상 도구(motion-studio)가 이 모양을 읽어 그래픽 틀을 바로 채웁니다. 모양이 안 맞으면 그냥 글로 읽힙니다.
    큰 숫자: 제목 / 숫자 / 부제          막대: 제목 / 이름 값, 이름 값
    추세: 제목 / 시점 값, 시점 값 / 단위   순위: 제목 / 이름 값, 이름 값
    비교: A vs B / A: 특징 | B: 특징     흐름: 제목 / 단계 > 단계 > 단계
    목록: 제목 / 항목, 항목              체크: 제목 / 항목, 항목
    요약: 제목 / 줄, 줄, 줄  (훅 다음 씬에 핵심 3줄 — 대본에 있는 말로)
  아래는 지면(에디토리얼) 모양입니다. 값이 다 있을 때만 쓰고, 이름 뒤 * 는 주인공(강조)입니다.
    덤벨: 제목 / 2015→2025 / 이름 값→값*, 이름 값→값 / 단위: 억      (두 시점 격차)
    기울기: 제목 / 작년→올해 / 이름 값→값*, 이름 값→값 / 단위: %     (여러 항목이 두 시점 사이 어떻게 움직였나)
    전후: 제목 / 작년→올해 / 이름 값→값, 이름 값→값                 (전후 표 + 차이)
    누적: 제목 / 몫1, 몫2, 몫3 / 이름 값 값 값, 이름 값 값 값         (100% 막대 — 몫 합이 100)
    추세표: 제목 / 이름* 값 값 값 값, 이름 값 값 값 값 / 단위: %      (지역마다 작은 추세선)
    전국지도: 제목 / 서울 3.1, 경기 1.8, 강원 -1.2 / 빗금: 강원 / 단위: %
    크기: 제목 / 이름 값, 이름 값 / 단위: 조                           (넓이로 크기 비교)
    범위: 제목 / 1월 값, 2월 값, 3월 값 / 최저: 값 값 값 / 최고: 값 값 값 / 단위: 만 건   (지난 몇 해 범위 띠 위 올해 — 최저·최고 칸 수는 시점 수와 같게)
    달력: 제목 / 9/1 값, 9/2 값, 9/3 값 / 단위: 건                    (날짜별 값)
  수치 모양(큰 숫자·막대·추세·순위와 지면 모양) 끝에는 `/ 출처: 기관` 을 붙입니다. 값과 기관은 **위 자료에 있는 것만** 씁니다.
  퍼센트가 아닌 값(표 수·건수)은 막대로 적고, 모양이 필요 없는 씬(훅·전환·마무리)은 억지로 붙이지 말고 글로만 적습니다.
  예: "큰 숫자: 서울 아파트값 / 14.56% / 최근 1년 상승률 / 출처: 한국부동산원 + 숫자가 올라온 뒤 '그런데 계약은?' 물음이 뒤따름".
- **사진 한 칸 (선택 · 영상 전체에 2곳까지).** 실물이 보이면 이해가 빨라지는 말(특정 단지·건물·장소·제품·문서)이 있는 씬에만,
  그 씬 visual 끝에 ` + 사진(낱말): 어떤 사진` 을 붙입니다. 괄호 안은 **그 씬 자막에 실제로 있는 말 그대로** 적습니다 — 그 말이 읽히는 순간 사진이 화면 한 칸에 들어옵니다.
  파일 이름은 지어내지 말고 어떤 사진인지만 짧게 적습니다 (사진은 제작자가 직접 구해 넣고, 안 넣으면 그래픽만으로 갑니다 —
  그러니 사진이 없어도 씬이 서도록 그래픽 지시는 그대로 다 적습니다). 사진이 씬의 주인공이 되면 안 됩니다. 마땅한 자리가 없으면 넣지 마세요.
  예: "큰 숫자: 재건축 분담금 / 4억 2천만원 / 조합원 평균 / 출처: 서울시 + 사진(은마아파트): 은마아파트 단지 전경".
