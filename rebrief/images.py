...
def trade_volume_bar(data: dict, date: str, extra: dict | None = None) -> "Image | None":
    ...
    notes = ["※ 해제(계약 취소) 신고분 제외"
             + (f" · 신고 기한(계약 후 30일)이 지난 {vol['days']}일까지 계약분" if vol.get("window") else "")]
    notes.append(f"출처 국토교통부 실거래가 · {date} 집계")
    w = 1000
    row_h = 60
...
def district_choropleth(data: dict, date: str, extra: dict | None = None, *,
    ...
    if metric == "count" and source.get("window"):
        ...
        notes.append(f"※ 신고 기한(계약 후 30일)이 지난 {source['days']}일까지 계약분")
    notes.append(f"출처 국토교통부 실거래가 · {date} 집계")
...
def cards(brief: dict, *, date: str = "", channel: str = "", key_numbers: list[dict] | None = None,
    ...
    **모델을 새로 부르지 않습니다.** 이미 만들어 둔 브리핑(headline·issues·numbers·
    ...
    짙음과 밝음이 교차하며 묶음에 박자를 만듭니다. '내일 볼 것' 장은 2026-09-09 에
